...
void xored_memcpy(uint8_t *dst, const uint8_t *a, const uint8_t *b, int bytelen) {
    #pragma unroll
...
void ClefiaConSet(uint8_t *con, const uint8_t *iv, int lk) {
    uint8_t t[2];
    uint8_t tmp;
...
void ClefiaGfn4(uint8_t *y, const uint8_t *x, const uint8_t *rk, int r) {
...
void ClefiaGfn8(uint8_t *y, const uint8_t *x, const uint8_t *rk, int r) {
...
void clefia_128_set_key(uint8_t *user_key, uint8_t *key_enc, uint8_t *key_dec) {
    uint8_t *whitening_key_enc = key_enc;
    uint8_t *whitening_key_dec = key_dec;
    uint8_t *round_key_enc = key_enc + 16;
    uint8_t *round_key_dec = key_dec + 16;
    ...
    memcpy(skey128, user_key, 16);
    /* generating CONi^(128) (0 <= i < 60, lk = 30) */
    ClefiaConSet(con128, iv, 30);
    /* GFN_{4,12} (generating L from K) */
    ClefiaGfn4(lk, skey128, con128, 12);

    memcpy(whitening_key_enc, skey128, 8); /* initial whitening key (WK0, WK1) */
    ...
    for (int i = 0; i < 9; i++) { /* round key (RKi (0 <= i < 36)) */
        dual_xored_memcpy(rk_enc, rk_dec, lk, con128 + i * 16 + (4 * 24), 16);
        if (i % 2) {
            dual_xored_memcpy(rk_enc, rk_dec,  rk_enc, skey128, 16); /* Xoring K */
        }
        ClefiaDoubleSwap(lk); /* Updating L (DoubleSwap function) */
        rk_enc += 16;
        rk_dec -= 16;
    }
    memcpy(whitening_key_enc + 8, skey128 + 8, 8); /* final whitening key (WK2, WK3) */
    memcpy(whitening_key_dec, skey128 + 8, 8); /* initial whitening key (WK2, WK3) */
}

void clefia_192_256_set_key(uint8_t *user_key, uint8_t *key_enc, uint8_t *key_dec, size_t keylen) {
    uint8_t *whitening_key_enc = key_enc;
    uint8_t *whitening_key_dec = key_dec;
    uint8_t *round_key_enc = key_enc + 16;
    uint8_t *round_key_dec = key_dec + 16;

    uint8_t iv[2] = {0x71U, 0x37U}; /* cubic root of 3 */
    if (keylen > 192) {
        iv[0] = 0xb5U; iv[1] = 0xc0U;
    }
    ...
    if (keylen > 192) {
        memcpy(skey256, user_key, 32);
        ClefiaConSet(con, iv, 46);
    } else {
        memcpy(skey256, user_key, 24);
        #pragma unroll
        for (int i = 0; i < 8; i++) {
            skey256[i + 24] = ~user_key[i];
        }
        ClefiaConSet(con, iv, 42);
    }

    /* GFN_{8,10} (generating L from K) */
    ClefiaGfn8(lk, skey256, con, 10);

    xored_memcpy(whitening_key_enc,
                 skey256, skey256 + 16, 8); /* initial whitening key (WK0, WK1) */
    xored_memcpy(whitening_key_dec + 8,
    ...
    for (int i = 0; i < 11; i++) { /* round key (RKi (0 <= i < 44)) */
        if ((i / 2) % 2) {
            dual_xored_memcpy(rk_enc, rk_dec, lk + 16, con + i * 16 + (4 * 40), 16); /* LR */
            if (i % 2) {
                dual_xored_memcpy(rk_enc, rk_dec, rk_enc, skey256 + 0,  16); /* Xoring KL */
            }

            ClefiaDoubleSwap(lk + 16); /* updating LR */
        } else {
            dual_xored_memcpy(rk_enc, rk_dec, lk + 0,  con + i * 16 + (4 * 40), 16); /* LL */
            if (i % 2) {
                dual_xored_memcpy(rk_enc, rk_dec, rk_enc, skey256 + 16, 16); /* Xoring KR */
            }
    ...
    xored_memcpy(whitening_key_enc + 8,
                 skey256 + 8, skey256 + 24, 8); /* final whitening key (WK2, WK3) */
    xored_memcpy(whitening_key_dec,
                 skey256 + 8, skey256 + 24, 8); /* initial whitening key (WK2, WK3) */
}


void clefia_expandkey(uint8_t *user_key, uint8_t *key_enc, uint8_t *key_dec, size_t keylen) {
    if (keylen == 128) {
        clefia_128_set_key(user_key, key_enc, key_dec);
    } else {
