...
void init_aes_methods_and_state(CipherFamily* fam) {
    ...
    fam->methods[AES_128_ECB_ENC] = CipherMethod_init(fam, "aesEncCipher", 0);
    fam->methods[AES_192_ECB_ENC] = CipherMethod_init(fam, "aesEncCipher", 0);
    fam->methods[AES_256_ECB_ENC] = CipherMethod_init(fam, "aesEncCipher", 0);
    ...
    fam->methods[AES_128_CTR] = CipherMethod_init(fam, "aesCipherCtr", 2);
    fam->methods[AES_192_CTR] = CipherMethod_init(fam, "aesCipherCtr", 2);
    fam->methods[AES_256_CTR] = CipherMethod_init(fam, "aesCipherCtr", 2);
...
CipherFamily* get_aes_family(struct OpenCLEnv* environment) {
    char *kernel_path = ParamAtlas_aget_full_kernel_path(environment->parameters, "aes_2ch");
