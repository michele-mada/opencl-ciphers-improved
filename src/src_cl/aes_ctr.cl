...
void debug_round(size_t r, uint temp_state[NUM_WORDS], __global uint *w) {
    printf("Round %lu\n", r);
    printf("key used: [%X, %X, %X, %X]\n", w[0], w[1], w[2], w[3]);
    printf("state: [%X, %X, %X, %X]\n", temp_state[0], temp_state[1], temp_state[2], temp_state[3]);
}
...
void encrypt(__private uchar state_in[BLOCK_SIZE],
    ...
    addRoundKey(temp_state1, w, 0);

    for (size_t r = 0; r < (num_rounds - 2) >> 1; r++) {
        AES_LINEAR_ENC_ROUND(temp_state2, temp_state1);
        addRoundKey(temp_state2, w, (r * NUM_WORDS * 2) + NUM_WORDS);

        AES_LINEAR_ENC_ROUND(temp_state1, temp_state2);
        addRoundKey(temp_state1, w, (r * NUM_WORDS * 2) + (NUM_WORDS * 2));
    }
    AES_LINEAR_ENC_ROUND(temp_state2, temp_state1);
    addRoundKey(temp_state2, w, (num_rounds << 2) - NUM_WORDS);

    AES_LINEAR_ENC_ROUND_FINAL(temp_state1, temp_state2);
    addRoundKey(temp_state1, w, num_rounds << 2);
...
__kernel void aesEncCipher(__global uchar* in,
    ...
    for (size_t i = 0; i < BLOCK_SIZE; ++i) {
        size_t offset = gid * BLOCK_SIZE + i;
        state[i] = in[offset];
    }

    /* call encrypt and get output */
    __private uchar outCipher[BLOCK_SIZE];
    encrypt(state, w, outCipher, num_rounds);
...
__kernel void aesDecCipher(__global uchar* in,
    ...
    /* call decrypt and get output */
    __private uchar outCipher[BLOCK_SIZE];
    decrypt(state, w, outCipher, num_rounds);
