...
ushort FI(ushort FI_IN, ushort FI_KEY) {
    ushort d9; // 9-bit integer;
    uchar  d7; // 7-bit integer;
    ...
    ushort FI_OUT = (d7<<9) | d9;
    return FI_OUT;
}

uint FO(uint FO_IN, ushort *EK, unsigned k) {
    ushort t0;
    ushort t1;
    ...
    uint FO_OUT = (t1<<16) | t0;
    return FO_OUT;
}

uint FL_EVEN(uint FL_IN, ushort *EK, unsigned k) {
    ushort d0 = FL_IN >> 16;
    ushort d1 = FL_IN & 0xffff;

    d1 = d1 ^ (d0 & EK[k/2]);
    d0 = d0 ^ (d1 | EK[(k/2+6)%8+8]);

    uint FL_OUT = (d0 << 16) | d1;

    return FL_OUT;
}

uint FL_ODD(uint FL_IN, ushort *EK, unsigned k) {
    ushort d0 = FL_IN >> 16;
    ushort d1 = FL_IN & 0xffff;
...
uint FLINV_EVEN(uint FL_IN, ushort *EK, unsigned k) {
    ushort d0 = FL_IN >> 16;
    ushort d1 = FL_IN & 0xffff;
...
uint FLINV_ODD(uint FL_IN, ushort *EK, unsigned k) {
    ushort d0 = FL_IN >> 16;
    ushort d1 = FL_IN & 0xffff;
...
// NOTE: the roundnum passed to the following macros must be **Even** !

#define DOUBLE_ROUND_ENC(roundnum)                  \
{                                                   \
    temp0 = FL_EVEN(D0, key, (roundnum));           \
    temp1 = FL_ODD(D1, key, (roundnum)+1);          \
    D1 = temp1 ^ FO(temp0, key, (roundnum));        \
                                                    \
    D0 = temp0 ^ FO(D1, key, (roundnum)+1);         \
}

#define ENC_ROUND_FINAL(roundnum)                   \
{                                                   \
    D0 = FL_EVEN(D0, key, (roundnum));              \
    D1 = FL_ODD(D1, key, (roundnum)+1);             \
}

#define DOUBLE_ROUND_DEC(roundnum)                  \
{                                                   \
    temp0 = D0 ^ FO(D1, key, (roundnum)+1);         \
    temp1 = D1 ^ FO(temp0, key, (roundnum));        \
    D0 = FLINV_EVEN(temp0, key, (roundnum));        \
    D1 = FLINV_ODD(temp1, key, (roundnum)+1);       \
}

#define DEC_ROUND_INITIAL(roundnum)                 \
{                                                   \
    D0 = FLINV_EVEN(D0, key, 8);                    \
    D1 = FLINV_ODD(D1, key, 9);                     \
}


void encrypt(__private uchar state_in[BLOCK_SIZE],
             __private ushort* key,
             __private uchar state_out[BLOCK_SIZE]) {
    ulong *state_in64 = (ulong *) state_in;
    ulong *state_out64 = (ulong *) state_out;

    uint D0 = (*state_in64) >> 32;
    uint D1 = (*state_in64) & 0xffffffff;
    uint temp0, temp1;
...
void decrypt(__private uchar state_in[BLOCK_SIZE],
    ...
    uint D1 = (*state_in64) >> 32;
    uint D0 = (*state_in64) & 0xffffffff;
    uint temp0, temp1;
    ...
    (*state_out64) = ((ulong)D0<<32) | D1;
}


#define ENCRYPT_INTERFACE(in, key, out) encrypt(in, (ushort*)key, out)
#define DECRYPT_INTERFACE(in, key, out) decrypt(in, (ushort*)key, out)


__attribute__((reqd_work_group_size(1, 1, 1)))
__kernel void misty1CipherEnc(__global uchar* restrict in,
                              __global uchar* restrict keys,
                              __global uchar* restrict out,
                              unsigned int num_rounds,
                              unsigned int input_size) \
    ECB_MODE_BOILERPLATE(ENCRYPT_INTERFACE, in, out, keys, BLOCK_SIZE, MAX_EXKEY_SIZE_WORDS*2, input_size);

__attribute__((reqd_work_group_size(1, 1, 1)))
__kernel void misty1CipherDec(__global uchar* restrict in,
                              __global uchar* restrict keys,
                              __global uchar* restrict out,
                              unsigned int num_rounds,
                              unsigned int input_size) \
    ECB_MODE_BOILERPLATE(DECRYPT_INTERFACE, in, out, keys, BLOCK_SIZE, MAX_EXKEY_SIZE_WORDS*2, input_size);


__attribute__((reqd_work_group_size(1, 1, 1)))
__kernel void misty1CipherCtr(__global uchar* restrict in,
                              __global uchar* restrict keys,
                              __global uchar* restrict out,
                              __global uchar* restrict IV,
                              unsigned int num_rounds,
                              unsigned int input_size) \
    CTR_MODE_BOILERPLATE(ENCRYPT_INTERFACE, in, out, keys, IV, BLOCK_SIZE, MAX_EXKEY_SIZE_WORDS*2, input_size);
