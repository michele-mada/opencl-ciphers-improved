...
#define CAST5_ROUND_AND_SWAP(num, fnum, block1, block2) \
{ \
    uint Km = key[(num)]; \
    uchar Kr = key[16 + (num)] & ((1u << 5) - 1); \
    temp = (block2); \
    (block2) = (block1) ^ f##fnum((block2), Km, Kr); \
    (block1) = temp; \
}
...
void encrypt_12round(__private uchar state_in[BLOCK_SIZE],
    ...
    CAST5_ROUND_AND_SWAP(2, 3, L, R);
    CAST5_ROUND_AND_SWAP(3, 1, L, R);
    CAST5_ROUND_AND_SWAP(4, 2, L, R);
    CAST5_ROUND_AND_SWAP(5, 3, L, R);
    CAST5_ROUND_AND_SWAP(6, 1, L, R);
    CAST5_ROUND_AND_SWAP(7, 2, L, R);
    CAST5_ROUND_AND_SWAP(8, 3, L, R);
...
__attribute__((reqd_work_group_size(1, 1, 1)))
__kernel void cast5CipherCtr(__global uchar* restrict in,
                             __global uint* restrict keys,
                             __global uchar* restrict out,
